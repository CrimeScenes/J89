...
local function isWithinViewArea(position)
    local screenPos = Camera:WorldToViewportPoint(position)
    local screenCenter = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y / 2)
    local viewHeight = getgenv().ViewArea.VerticalSize * 100
    local viewWidth = getgenv().ViewArea.HorizontalSize * 100
    return (Vector2.new(screenPos.X, screenPos.Y) - screenCenter).Magnitude <= math.sqrt((viewHeight / 2)^2 + (viewWidth / 2)^2)
end

-- Get all the body parts of a character
...
-- Smooth mouse movement (Revised method)
local function smoothMouseMovement(targetPos)
    local currentX, currentY = getMousePosition()
    local targetVector = Vector2.new(targetPos.X, targetPos.Y)
    local currentVector = Vector2.new(currentX, currentY)
    local direction = (targetVector - currentVector).unit
    local distance = (targetVector - currentVector).Magnitude

    -- Apply smooth factor for gradual movement, dynamic adjustment based on distance
    local moveSpeed = math.min(distance * 0.15, 5) -- Limit speed to avoid overshoot
    local newPos = currentVector + direction * moveSpeed
...
-- Check if the player is ignoring the knife tool
...
-- Triggerbot action logic with better timing and movement smoothing
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then
            if isMouseOnTarget(TargetPlayer) then
                ...
                for _, part in pairs(bodyParts) do
                    ...
                    if onScreen and isWithinViewArea(part.Position) then
                        smoothMouseMovement(screenPos) -- Apply smooth mouse movement
                        if tick() - lastClickTime >= 0.05 then  -- Use tick() for more consistent timing
                            lastClickTime = tick()
...
-- Continuous triggerbot action when toggled
RunService.RenderStepped:Connect(function()
    if isToggled then
        TriggerBotAction()
    end
